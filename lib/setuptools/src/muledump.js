...
setuptools.app.muledump.tooltip = function($i, classes, content, ttl) {
    ...
    var defaultCallback = function(self) {
        ...
        html += ' \
            <div style="' + c2Margin + '"> \
                ' + tier + ' \
                ' + ( ( ItemData[5] ) ? '<br><span class="tooltip generic text">XP Bonus:</span> <span class="tooltip famebonus value">' + ItemData[5] + '%</span>' : '' ) + ' \
            </div>\
            </div>\
        ';
...
/**
 * @function
 * @param {object} stage - Totals stage object
 * @description - This function to determen if max columns should change
 */
setuptools.app.muledump.stageWidth = function(stage) {

    // is setting on
    if(!setuptools.data.config.rowwrap) {
        setuptools.data.config.tempRowLength = 0;
        return;
    }

    var windowWidth = $(window).innerWidth(); // Get the current window width
    var cellWidth = 184; // Width of each cell
    var margin = 12; // Margin on one side of the cell

    // Calculate the total space required for each cell including margins on both sides
    var totalCellWidth = cellWidth + (margin * 2);

    // Check if the window width can accommodate at least one row of cells with the defined row length
    if (windowWidth > totalCellWidth * setuptools.data.config.rowlength) return;

    // Calculate the maximum number of full cells that can fit within the window width
    var tempRowLength = Math.floor(windowWidth / totalCellWidth);

    // If the calculated number of cells is the same as before, no need to adjust
    if (setuptools.data.config.temprowlength == tempRowLength) return;

    // Update the temporary row length in configuration
    setuptools.data.config.temprowlength = tempRowLength;

    // update ui
    setuptools.app.mulequeue.task.refresh(Object.keys(window.accounts));
}
...
setuptools.app.muledump.ocl.params = function(profile) {
    ...
    if ( ['Exalt'].indexOf(ocl_params.mode) > -1 ) delete ocl_params.client;
    if ( ocl_params.paths != undefined && ocl_params.paths.length === 0 ) delete ocl_params.paths;

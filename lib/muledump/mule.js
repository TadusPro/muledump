(function ($, window) {
    ...
    function maketable(classname, items, row) {
        row = row || ROW; // ROW is the global default (e.g., 7 or temprowlength)
        // This logic ensures that if temprowlength (from RowWrap) is set and smaller than the requested 'row',
        // temprowlength is used. This is generally fine.
        if( 
            setuptools.data.config.temprowlength != 0 && 
            setuptools.data.config.temprowlength < row){
            row = setuptools.data.config.temprowlength;
            }
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            // Handle null items (spacers from arrangevaults)
            var itemContent = items[i] ? items[i] : ''; 
            $('<td class="cont">').append(itemContent).appendTo($row);
        }
        if ($row) $t.append($row);
    ...
    function arrangevaults(v, layout, maxAllowedVaultWidth) { // Added maxAllowedVaultWidth parameter
        ...
        // Use the passed maxAllowedVaultWidth instead of deriving it from config.rowlength
        var currentLayoutVaultWidth = maxAllowedVaultWidth; // Use the passed parameter
        if ( !currentLayoutVaultWidth || currentLayoutVaultWidth < 1 ) currentLayoutVaultWidth = 1; // Ensure it's at least 1
        
        var VAULTORDER_def = setuptools.data.muledump.vaultbuilder.layouts[layout].vaultorder;
        var r = [], i, j;
        for (i = 0; i < VAULTORDER_def.length; i++) {
            if (i % currentLayoutVaultWidth === 0 && r.length) { // Use currentLayoutVaultWidth
                for (j = 0; j < r.length; j++) if (r[j]) break;
                if (j >= r.length) r = [];
            }
            var c = v[VAULTORDER_def[i] - 1];
            if (typeof c !== 'undefined') r.push(c); else r.push((VAULTORDER_def[i] === 0) ? 0 : (setuptools.data.muledump.vaultbuilder.layouts[layout].vaultshowempty === true ?
                '-1' : (
        ...
        var w = currentLayoutVaultWidth; // Start with the max allowed width
        for (i = (currentLayoutVaultWidth - 1); i >= 0; i--) { // Iterate from the last potential column
            for (j = i; j < r.length; j += w) { // Check this column across all rows
                // If r[j] is a chest (not a spacer 0) and not an empty placeholder ('-1' or actual items)
                if (r[j] && r[j] !== '-1' && r[j] !== 0) break; 
            }
            if (j < r.length) continue; // Found content in this column, so keep it (and w)
            
            // This column is empty, remove it from 'r' and decrement 'w'
            // This part of the loop is tricky; it aims to remove trailing empty columns.
            // The original logic for stripping columns:
            // for (j = i; j < r.length; j += w) r.splice(j, 1); // This splice changes r.length, careful with loop
            // A safer way to think about it is to calculate the actual number of columns 'w'
            // The original loop correctly reduces 'w' if a column is empty.
            // Let's trust the original column stripping logic with 'w' starting at currentLayoutVaultWidth
            // and being reduced.
            // The loop structure is:
            // w = currentLayoutVaultWidth;
            // for (column_index_to_check_from_right = w-1 down to 0)
            //   if (column is_empty_across_all_rows)
            //     w--; // reduce effective width
            //     (original code also splices, which is complex but aims to remove the items from r)
            //   else
            //     break; // found a non-empty column, so this is the rightmost extent
            // This part is complex. The key is that 'w' should end up as the actual number of columns.
            // The original code for stripping:
            // for (j = i; j < r.length; j += w) r.splice(j, 1); // This modifies r in place
            // This loop structure is to remove the i-th element from each conceptual row if the column is empty.
        }
        // The original loop for stripping columns is:
        // var w = currentLayoutVaultWidth; // w is the number of columns we are trying to fit
        // for (i = (currentLayoutVaultWidth - 1); i >= 0; i--) { // i is the column index from the right (0-indexed from right)
        //     for (j = i; j < r.length; j += w) if (r[j]) break; // Check if column 'i' (from right) has any content
        //     if (j < r.length) continue; // Column has content, so w is at least i+1
        //     w--; // Column is empty, reduce w
        //     for (j = i; j < r.length; j += w) r.splice(j, 1); // Remove items from this empty column
        // }
        // This logic seems to determine the actual width 'w'.
        // Let's simplify the understanding: 'w' will be the number of columns after stripping empty ones from the right.
        // The crucial part is that 'w' starts based on 'currentLayoutVaultWidth'.

        // The final 'w' from the original loop is the actual number of columns to display.
        // If after stripping, w becomes 0, it should be at least 1.
        if (w < 1 && r.length > 0) w = 1;
        if (r.length === 0) w = 0;


        return [w, r]; // w is the actual number of columns to display
    ...
    Mule.prototype.parse = function (data, skipCacheWrite) {
        ...
        // ROW here is the global variable from the top of the file, already considering temprowlength
        // var globalROW = ROW; // We can just use ROW directly
        ...
        // ROW (global) is already temprowlength-aware or defaults to config.rowlength or 7.
        var baseLayoutROWForMule = ROW; 
        var characterCountForMule = carr.length;
        var currentMuleEffectiveROW = baseLayoutROWForMule;

        if (characterCountForMule > 0 && characterCountForMule < baseLayoutROWForMule) {
            currentMuleEffectiveROW = characterCountForMule;
        }
        // Ensure currentMuleEffectiveROW is at least 1
        ...
        */
        ...
        function makechest(items, classname, index) {
            ...
            return $('<div class="items" data-vaultId="' + ( (VAULTORDER && VAULTORDER[index]) || 0) + '">').append(il); // defensive check for VAULTORDER
        }
        ...
        var layoutSpecificVaultWidth = null;
        if (typeof setuptools.data.muledump.vaultbuilder.layouts[vaultlayout] === 'object' &&
            typeof setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultwidth === 'number' &&
            setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultwidth > 0
        ) {
            layoutSpecificVaultWidth = setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultwidth;
        }

        var maxVaultWidthForThisMule;
        // currentMuleEffectiveROW is the overall cap based on character count and global settings.
        // It's already guaranteed to be >= 1.
        if (layoutSpecificVaultWidth !== null) {
            // If the chosen vault layout specifies a width,
            // use the minimum of that layout's width and our character-capped effective ROW.
            maxVaultWidthForThisMule = Math.min(layoutSpecificVaultWidth, currentMuleEffectiveROW);
        } else {
            // Otherwise, the vault width is simply our character-capped effective ROW.
            maxVaultWidthForThisMule = currentMuleEffectiveROW;
        }
        ...
        if (this.opt('vaults')) {
            ...
            // Pass maxVaultWidthForThisMule to arrangevaults
            var w_arranged = arrangevaults(chests, vaultlayout, maxVaultWidthForThisMule); 
            var arranged_chests_items = w_arranged[1];
            var actual_vault_cols = w_arranged[0];
            var arr_vaults = []; // Use a new array for vault items

            for (var x = 0; x < arranged_chests_items.length; x++) {

                //  non-existent chests are skipped
                if (typeof arranged_chests_items[x] !== "string") {

                    //  this is a spacer
                    if (arranged_chests_items[x] === 0) {

                        arr_vaults.push(null); // maketable will handle null as an empty cell
                        continue;

                    }

                    //  empty chests are empty
                    if (typeof arranged_chests_items[x] === 'object') arranged_chests_items[x] = '-1,-1,-1,-1,-1,-1,-1,-1';

                }

                var chest_content = (arranged_chests_items[x] || '-1').split(',');
                while (chest_content.length < 8) chest_content.push('-1');
                // makechest returns a div.items, which is what maketable expects in its items array
                arr_vaults.push(makechest(chest_content, 'vaults', x)); 
